                ...
                {rowVals.map((key, index) => {
                    return (
                        <View key = {index} style={{flex:1, flexDirection: 'row', height: 30 + 10*this.props.colCount}}>
                            <TableCell id={index} flexVal={ this.props.colCount }>{rowVals[index]}</TableCell>
                            {Array.from(Array(this.props.colCount-1),(key2, index2) => {
                                return (
                                    <View key={index2} style={{flex: 1/this.props.colCount, borderStyle: 'solid', borderColor: '#CED0CE', borderRadius: 1, borderWidth: 0.5}}>
                                        <TouchableHighlight style={{height: 30 + 10*this.props.colCount, justifyContent: 'center', alignItems: 'center', backgroundColor: colors[(index + index2)%2] }}
